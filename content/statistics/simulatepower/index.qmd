...
n_metabolites <- 230
n_per_batch <- 6
true_prop_relevant <- 0.02 # two percent of all metabolites are relevant (230 * 0.02 = 4.6)
```
...
# Determine if a metabolite positively correlates with KYNA _if_ it is relevant
...
data <- data.frame(y = y, KYNA_t0 = KYNA_t0, group_index = group_index)
data$X <- X

# Define the formula
formul <- bf(y ~ 1 + KYNA_t0 + X + (1 | group_index))
# Define the priors
